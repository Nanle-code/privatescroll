...
| **On-chain edit history, made visible** | Every save after the first also proves the specific content transition and binds it to the author. The editor shows it as a real provenance timeline, not a plain list — every version links straight to `/verify` with that exact proof pre-checked, so "genuine work, not a paste-dump" is something you can click and see, not just read about. |
